...
## Correlation Test Between the Various Weight Variables
...
Null Hypothesis: There are no correlation between each pairwise set of variables
Alternative Hypothesis: There is a correlation between each pairwise set of variables.
...
```{r}
lu_corr <- lu.decomposition(correlation)
lu_corr
```
...
Using backward stepwise regression, we find the following model. The p values for all the independent variables are less than 0.05 which indicated they are strongly related to the dependent variable Age. The F-statisics is large. The adjusted R-squared is 0.5392, which means the model can explain 53.92% of variance in the crabs age. 

```{r}
model1 <- step(lm( Age ~ Length + Diameter + Height + Weight + 
    Shucked.Weight + Viscera.Weight + Shell.Weight, data = train), direction = "backward")
summary(model)
```
...
https://mathstat.slu.edu/~speegled/_book/SimulationRV.html
https://rpubs.com/finnstats/skewness-in-R
